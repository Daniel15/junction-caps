...
Handler.prototype._getExtensionFromPresence = function(stanza, ext) {
	...
	if (!this._pendingExt[fullNode][ext]) {
		...
		getExt.id = id;
		getExt.c('query', { xmlns: 'http://jabber.org/protocol/disco#info', node: stanza.caps.node + '#' + ext });
...
Handler.prototype._handleIQ = function (stanza) {
	...
	if (id.substr(0, 9) === 'caps-ext-') {
		this._handleIQExt(stanza);
	} else {
		this._handleIQCaps(stanza);	
	}
};
 ...
 * @api private
 */
Handler.prototype._handleIQExt = function(stanza) {
	var id = stanza.attrs.id,
		caps = stanza.capabilities,
 ...
 * @api private
 */
Handler.prototype._handleIQCaps = function(stanza) {
	var node = stanza.capabilities.node;
...
Handler.prototype._emitCapabilitiesEvent = function(presence) {
	...
	var caps = presence.caps,
		...
		// modify the original)
		mainCaps = JSON.parse(JSON.stringify(this._cache[fullNode]));
	...
	removeDuplicates(mainCaps.rawFeatures);
...
/**
 * Remove duplicate features from the specified feature array. Modifies the array in-place
