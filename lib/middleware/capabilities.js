...
function Handler() {
	...
	// TODO: Allow this to be passed in the constructor
...
Handler.prototype._handlePresence = function (stanza) {
	...
	self._cache.getCapabilities(fullNode, function capsCallback(err, cachedCaps) {
		if (err) throw err;
		self._cache.getExtensions(fullNode, caps.ext || [], function extsCallback(err, cachedExts, uncachedExts) {
			if (err) throw err;
			...
			if (cachedCaps && uncachedExts.length === 0) {
				this._emitCapabilitiesEvent(stanza, cachedCaps, cachedExts);
				return;
			}
...
Handler.prototype._handleIQExt = function(stanza, request) {
	...
	this._cache.saveExtension(fullNode, request.ext, caps.rawFeatures);
...
Handler.prototype._handleIQCaps = function(stanza, request) {
	...
	this._cache.saveCapabilities(node, caps);
...
Handler.prototype._emitEventIfFullyLoaded = function(jid) {
	var self = this,
		pendingBuddy = this._pendingBuddies[jid], 
		presence = pendingBuddy.originalPresence,
		fullCapsNode = presence.caps.node + '#' + presence.caps.ver;
	
	this._cache.getCapabilities(fullCapsNode, function(err, caps) {
		if (err) throw err;
		...
		if (caps && pendingBuddy.pendingExts.length === 0) {
			self._emitCapabilitiesEvent(presence);
		}	
	});
};
...
Handler.prototype._emitCapabilitiesEvent = function(presence) {
	...
	this._cache.getCapabilities(fullNode, function capsCallback(err, mainCaps) {
		if (err) throw err;
		
		self._cache.getExtensions(fullNode, caps.ext || [], function extsCallback(err, exts, uncachedExts) {
			if (err) throw err;
			
			// Create a deep clone of the capabilities (as we will be adding the extensions to them and don't want to 
			// modify the original)			
			mainCaps = capsUtil.deepClone(mainCaps);
			
			for (var i in exts) {
				if (exts.hasOwnProperty(i)) {
					mainCaps.rawFeatures = mainCaps.rawFeatures.concat(exts[i]);
				}
			}
			...
			self.emit('capabilities', presence.from, mainCaps);
		});		
	})
};